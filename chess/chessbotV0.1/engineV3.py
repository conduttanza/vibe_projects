...
def evaluate_protected_capture(move, board):
    if board.is_capture(move):
        ...
        if captured_piece:
            # Your piece making the move
            moving_piece = board.piece_at(move.from_square)

            # Check defenders of the destination square
            defenders = board.attackers(not board.turn, move.to_square)
            
            # If destination is defended
            if defenders and moving_piece:
                if piece_value(moving_piece.piece_type) <= piece_value(captured_piece.piece_type):
                    # Good capture: we are trading up or equal
                    return piece_value(captured_piece.piece_type)
                else:
                    # Bad capture (e.g., trading a rook for a pawn), avoid it
                    return 0
            else:
                # Square is *undefended*, freely capture
                return piece_value(captured_piece.piece_type)
    return 0
        ...
        if piece.color == chess.WHITE:
            if square in attacked_b_sq:
                if piece.piece_type in [chess.ROOK, chess.QUEEN]:
                    value -= int(piece_value(piece.piece_type) * 1.5)
                else:
                    value -= int(piece_value(piece.piece_type) * 0.9)
        if piece.color == chess.BLACK:
...
def get_best_move_minimax(board, time_limit=60):
